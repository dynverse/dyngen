...
params = list(
  model = list(
    ...
    damping = 0.05,
    ntargets_sampler = function() {sample(1:20, 1)}
  ),
  simulation = list(
    totaltime = 10,
    burntime = 0,
...
simulation <- invoke(simulate_multiple, params$simulation, model)
...
module_counts <- get_module_counts(experiment$expression, model$geneinfo)
