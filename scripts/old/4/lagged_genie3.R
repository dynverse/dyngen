...
genie3 <- function(data, regulators=seq_len(ncol(data)), targets=seq_len(ncol(data)), data.regulators=NULL,
  ...
  if (mc.cores == "qsub") {
    ...
    lapplyfun <- function(X, FUN, ...) PRISM::qsub_lapply(X = X, FUN = FUN, qsub.environment = c("data", "feature.names", "regulators", "nb.trees", "importance.measure", "regulator.names", "num.regulators", "data.regulators"), qsub_conf)
  } else if (mc.cores < 1) {
