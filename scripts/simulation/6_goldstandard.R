...
get_states_and_progression = function(expression_modules, modulenodes, expression_cutoff=3, expression_max=4) {
  modulesoi = modulenodes %>% filter(!is.na(state))
  
  expression_modules[, modulesoi$module] %>% apply(1, function(x) {
    state = (1:length(x))[x>expression_cutoff] %>% last %>% modulesoi$state[.]
    if (is.na(state)) {
...
get_branchconnected_statenet = function(statenet) {
  ...
  print(statenet$type)
  igraph::graph_from_data_frame(statenet[,c("from", "to")]) %>% plot(edge.color=factor(statenet$type))
  snet = statenet[,c("from", "to")] %>% graph_from_data_frame(vertices=unique(c(statenet$from, statenet$to)) %>% sort)
  list(snet=snet, statenet=statenet)
}
...
process_sp = function(sp, statenet) {
  extracost = 0
  if(length(sp) == 1) {
    traj_distance_samestate
  } else {
    edge = get_state_edge(sp[[1]], sp[[2]], statenet)
    ...
    edge = get_state_edge(sp[[length(sp)-1]], sp[[length(sp)]], statenet)
...
get_cell_distance_func = function(statenet, snet, state1, state2) {
  ...
  shortest_paths(snet, state1, state2, mode = "all")$vpath[[1]] %>% names() %>% as.integer() %>% process_sp(statenet)
}
#func = get_cell_distance_func(snet, 1, 2)
#func(0.9, 0.1)
#func(0.2, 0.3)

get_cell_distances = function(cellinfo, snet, statenet) {
  ...
  cell_distance_funcs = map(allstates, function(state1) map(allstates, ~get_cell_distance_func(statenet, snet, state1, .)))
  ...
  allcells = 1:nrow(dataset$expression)
