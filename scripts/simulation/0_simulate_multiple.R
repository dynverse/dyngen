...
experimentsettings = experimentsettings[1, ]

experiments = parallel::mclapply(seq_len(nrow(experimentsettings)), run_settingid, mc.cores = 1)

## Extract gold standard
gss = parallel::mclapply(experiments, function(experiment) {
  gs = dyngen:::extract_goldstandard(experiment, verbose=T)
  gs
}, mc.cores=4)

experiments = purrr::map2(experiments, gss, function(experiment, gs) {
  experiment$gs = gs
  experiment
})
