...
plot_piecestate_changes = function(dataset) {
  ...
  ggplot(ggnet, aes(x = x, y = y, xend = xend, yend = yend)) + 
    ...
    geom_nodes(aes(color=factor(piece)), size=10) +
    theme_blank()
  ...
  p = ggplot(ggnet2, aes(x = x, y = y, xend = xend, yend = yend, frame=start)) + 
    ...
    geom_nodes(aes(color=factor(piece), size=freq)) +
    geom_nodetext(aes(label = piece), size=9, alpha=0.2) +
    scale_size_continuous(range=c(1, 15)) +
    geom_nodes(aes(color=factor(piece)), size=15, alpha=0.2) +
    theme_blank()
  gg_animate(p)
}
