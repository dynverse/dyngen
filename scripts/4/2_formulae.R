generate_formulae = function(net, genes, celltypes=tibble(celltype=1, dies=F)) {
  ...
  add.variable = function(variable, ...) {
    ...
    if (group %in% c("b", "u", "x", "y", "k", "rg", "r", "d", "p", "q", "a0", "a")) {
  ...
  rgs = map(celltypes$celltype, ~add.variable(fvar(paste0("rg_", .)))) # global transcription rate
  ...
  tibble::lst(vargroups, variables, formulae.strings, nus.changes)
}
