...
model <- 
  initialise_model(
    ...
    backbone = backbone_branching(num_modifications = 2, min_degree = 3, max_degree = 4),
    tf_network_params = tf_network_random(min_tfs_per_module = 3),
    feature_network_params = feature_network_default(),
    kinetics_params = kinetics_custom(),
    gold_standard_params = gold_standard_default(),
    simulation_params = simulation_default(total_time = 10, num_simulations = 32, ssa_algorithm = ssa_em(noise_strength = 6)),
