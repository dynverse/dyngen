...
load_dataset = function(datasetid, contents = contents_dataset()) {
  combinedinfo = tibble(id=datasetid) %>% left_join(readRDS("results/datasets.rds"), by="id") %>% left_join(readRDS("results/goldstandards.rds") %>% select(-version) %>% rename(goldstandardid=id), by="experimentid") %>% left_join(readRDS("results/experiments.rds") %>% select(-version) %>% rename(experimentid=id), by="experimentid") %>% as.list()
