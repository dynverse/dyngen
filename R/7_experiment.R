...
generate_experiment <- function(model) {
  ...
  mol_info <- model$feature_info |>
    pivot_longer(cols = c("mol_premrna", "mol_mrna", "mol_protein"), names_to = "mol", values_to = "val") |>
...
.generate_experiment_snapshot <- function(
  ...
  steps_summ <- sim_meta |>
    ...
    summarise(num_steps = n(), .groups = "drop")
...
.generate_experiment_synchronised <- function(
  ...
  sim_meta2 |>
    slice(i) |>
    select(step_ix = "orig_ix", "timepoint_group")
}
