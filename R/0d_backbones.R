...
backbone_disconnected <- function(
  left_backbone = list_backbones() |>
    (\(bb) keep(bb, names(bb) != "disconnected"))() |>
    sample(1) |>
    first(),
  right_backbone = list_backbones() |>
    (\(bb) keep(bb, names(bb) != "disconnected"))() |>
    sample(1) |>
    first(),
  ...
  lep <- left$expression_patterns |>
    mutate_at(vars(from, to), leftpas) |>
    mutate(module_progression = gsub("([+-])", "\\1left_", module_progression))
  rep <- right$expression_patterns |>
    mutate_at(vars(from, to), rightpas) |>
    mutate(module_progression = gsub("([+-])", "\\1right_", module_progression))
