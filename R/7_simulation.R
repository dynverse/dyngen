...
#' @param compute_propensity_ratios Whether or not to compute the propensity ratios after simulation.
...
generate_cells <- function(model) {
  ...
  model$simulations <- lst(
    ...
    propensity_ratios = do.call(rbind, map(simulations, "propensity_ratios")),
...
.generate_cells_simulate_cell <- function(simulation_i, model, reactions, verbose = FALSE, debug = FALSE) {
  ...
  log_propensity <- sim_params$store_reaction_propensities || sim_params$compute_propensity_ratios
