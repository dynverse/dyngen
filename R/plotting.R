...
#' @importFrom ggplot2 ggsave labs aes coord_equal scale_colour_manual scale_size_manual scale_linewidth_manual facet_wrap geom_line geom_text ggplot theme_bw geom_path geom_point geom_step scale_linetype_manual
...
#' @return A ggplot2 object.
#'
#' @importFrom igraph layout_with_graphopt V E
#' @export
#'
#' @examples
#' \donttest{
...
plot_backbone_modulenet <- function(model) {
  ...
  layout <-
    gr |>
    igraph::layout_with_graphopt(charge = .01, niter = 10000) |>
  ...
  ggraph(gr, layout = "manual", x = layout$x, y = layout$y) +
    geom_edge_loop(aes(width = strength, strength = str, filter = effect >= 0 & from == to), arrow = arrow_up, start_cap = cap, end_cap = cap) +
    geom_edge_loop(aes(width = strength, strength = str, filter = effect < 0 & from == to), arrow = arrow_down, start_cap = cap, end_cap = cap) +
    geom_edge_fan(aes(width = strength, filter = effect >= 0 & from != to), arrow = arrow_up, start_cap = cap, end_cap = cap) +
    geom_edge_fan(aes(width = strength, filter = effect < 0 & from != to), arrow = arrow_down, start_cap = cap, end_cap = cap) +
...
plot_feature_network <- function(
  ...
  feature_network <-
    bind_rows(
      ...
      feature_info |>
        ...
        group_by(module_id) |>
        reframe(crossing(from = feature_id, to = feature_id) |> mutate(effect = -2)) |>
        filter(from < to)
    )
  ...
  ggraph(gr, layout = "manual", x = layout$x, y = layout$y) +
    ...
    geom_edge_loop(aes(strength = str, filter = is.na(effect) & from == to)) +
    geom_edge_fan(aes(filter = !is.na(effect) & effect >= 0 & from != to), arrow = arrow_up, start_cap = cap, end_cap = cap) +
    geom_edge_fan(aes(filter = !is.na(effect) & effect < 0 & from != to), arrow = arrow_down, start_cap = cap, end_cap = cap) +
    geom_edge_fan(aes(filter = is.na(effect) & from != to)) +
...
#' }
plot_simulations <- function(model, mapping = aes(.data$comp_1, .data$comp_2)) {
  plot_df <-
    bind_cols(
...
plot_gold_simulations <- function(model, detailed = FALSE, mapping = aes(.data$comp_1, .data$comp_2), highlight = 0) {
  plot_df <-
    bind_cols(
  ...
  ggplot(mapping = mapping) +
    ...
    geom_path(aes(colour = .data$edge, group = .data$group), plot_df |> filter(.data$simulation_i == highlight), linewidth = 2) +
    theme_bw() +
    labs(colour = "Edge")
}
...
#' }
plot_gold_mappings <- function(model, selected_simulations = NULL, do_facet = TRUE, mapping = aes(.data$comp_1, .data$comp_2)) {
  plot_df <-
    bind_rows(
      bind_cols(
        model$simulations$meta,
...
plot_gold_expression <- function(
  ...
  molecules <- model$feature_info |>
    filter(is_tf) |>
    pivot_longer(cols = c(!!!what), names_to = "mol", values_to = "val") |>
    pull(val)
  df <- bind_cols(
    model$gold_standard$meta,
    as.data.frame(as.matrix(model$gold_standard$counts))[, molecules]
  ) |>
    pivot_longer(cols = any_of(molecules), names_to = "molecule", values_to = "value") |>
    mutate(edge = factor(paste0(from_, "->", to_), levels = edge_levels)) |>
    left_join(model$feature_info |> select(module_id, !!!what) |> pivot_longer(cols = c(!!!what), names_to = "type", values_to = "molecule"), by = "molecule") |>
  ...
  g <- ggplot(df, aes(sim_time, value, colour = module_id)) +
    ...
    scale_linewidth_manual(values = c(mol_premrna = .5, mol_mrna = 1, mol_protein = .5)) +
...
plot_simulation_expression <- function(
  ...
  df <- bind_cols(
    ...
    left_join(
      model$feature_info |>
        ...
        pivot_longer(cols = c(!!!what), names_to = "type", values_to = "molecule") |>
  ...
  g <- ggplot(df, aes(sim_time, value)) +
    geom_step(aes(linetype = type, linewidth = type, colour = module_id)) +
    scale_linewidth_manual(values = c(mol_premrna = .5, mol_mrna = 1, mol_protein = .5)) +
    scale_colour_manual(values = model$backbone$module_info |> select(module_id, color) |> deframe()) +
    theme_bw() +
    labs(colour = "Module id", linetype = "Molecule type")
  ...
  if (label_nonzero) {
    ...
    df_labels <-
      df |>
      group_by(module_id, type, module_group) |>
      reframe({
        approx(x = sim_time, y = value, xout = pts) |>
          as_tibble() |>
...
#' }
plot_experiment_dimred <- function(model, mapping = aes(.data$comp_1, .data$comp_2)) {
