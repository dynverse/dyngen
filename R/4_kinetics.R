...
.kinetics_extract_parameters_as_df <- function(feature_info, feature_network) {
  ...
  feature_params <-
    feature_info |>
    select(
      "feature_id", "transcription_rate", "splicing_rate", "translation_rate",
      "mrna_decay_rate", "protein_decay_rate",
      bas = "basal", ind = "independence"
    ) |>
    pivot_longer(cols = -all_of("feature_id"), names_to = "param", values_to = "value") |>
  ...
  edge_params <-
    ...
    pivot_longer(cols = -all_of(c("from", "to")), names_to = "param", values_to = "value") |>
